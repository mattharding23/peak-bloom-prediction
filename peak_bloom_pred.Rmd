...
## Create function that creates variables to train a model with
...
train_data <- dc_bloom[training_samples, 7:40]
test_data <- dc_bloom[-training_samples, 7:40]
...
```{r}
x_test <- model.matrix(bloom_doy ~ ., ky_bloom[, 7:40])[, -1]
...
train_data <- blooms[training_samples, c(2,4:5, 7:40)]
test_data <- blooms[-training_samples, c(2,4:5, 7:40)]
...
```{r}
all_loc = rbind(test_data,vc_bloom[,c(2,4:5, 7:40)])
...
### Run model for DC
```{r}
x_test <- model.matrix(bloom_doy ~ ., dc_bloom_predictors[,c(2,4:5, 7:40)])[, -1]
...
### Run model
```{r}
x_test <- model.matrix(bloom_doy ~ ., ky_bloom_predictors[,c(2,4:5, 7:40)])[, -1]
...
### Run Model
```{r}
x_test <- model.matrix(bloom_doy ~ ., vc_bloom_predictors[,c(2,4:5, 7:40)])[, -1]
...
### Run model
```{r}
x_test <- model.matrix(bloom_doy ~ ., li_bloom_predictors[,c(2,4:5, 7:40)])[, -1]
