...
#'   location and a year and calculates variables to use in model
  ...
  # Cumulative weather data from the first 30 days of the year
  bloom$first_30_avgT <- sum(weather$tavg_K[1:30])
  bloom$first_30_maxT <- sum(weather$tmax_K[1:30])
  bloom$first_30_minT <- sum(weather$tmin_K[1:30])
  # bloom$first_30_precip = sum(weather$precip_mm[1:30])

  # Cumulative weather data from the first 60 days of the year
  bloom$first_60_avgT <- sum(weather$tavg_K[1:60])
  bloom$first_60_maxT <- sum(weather$tmax_K[1:60])
  bloom$first_60_minT <- sum(weather$tmin_K[1:60])
  # bloom$first_60_precip = sum(weather$precip_mm[1:60])
  
  # Cumulative weather data from the first 70 days of the year
  bloom$first_70_avgT <- sum(weather$tavg_K[1:70])
  bloom$first_70_maxT <- sum(weather$tmax_K[1:70])
  bloom$first_70_minT <- sum(weather$tmin_K[1:70])
  # bloom$first_70_precip = sum(weather$precip_mm[1:70])
  
  # Cumulative weather data from the first 80 days of the year
  bloom$first_80_avgT <- sum(weather$tavg_K[1:80])
  bloom$first_80_maxT <- sum(weather$tmax_K[1:80])
  bloom$first_80_minT <- sum(weather$tmin_K[1:80])
  # bloom$first_80_precip = sum(weather$precip_mm[1:680])

  # Cumulative weather data from the first 90 days of the year
  bloom$first_90_avgT <- sum(weather$tavg_K[1:90])
  bloom$first_90_maxT <- sum(weather$tmax_K[1:90])
  bloom$first_90_minT <- sum(weather$tmin_K[1:90])
  # bloom$first_90_precip = sum(weather$precip_mm[1:90])
  
  # Cumulative weather data from the first 90 days of the year
  bloom$first_100_avgT <- sum(weather$tavg_K[1:100])
  bloom$first_100_maxT <- sum(weather$tmax_K[1:100])
  bloom$first_100_minT <- sum(weather$tmin_K[1:100])
  # bloom$first_100_precip = sum(weather$precip_mm[1:100])
  
  # Cumulative weather data from the first 90 days of the year
  bloom$first_110_avgT <- sum(weather$tavg_K[1:110])
  bloom$first_110_maxT <- sum(weather$tmax_K[1:110])
  bloom$first_110_minT <- sum(weather$tmin_K[1:110])
  # bloom$first_110_precip = sum(weather$precip_mm[1:110])
  
  # Cumulative weather data from the first 90 days of the year
  bloom$first_120_avgT <- sum(weather$tavg_K[1:120])
  bloom$first_120_maxT <- sum(weather$tmax_K[1:120])
  bloom$first_120_minT <- sum(weather$tmin_K[1:120])
  # bloom$first_110_precip = sum(weather$precip_mm[1:110])

  # Cumulative weather data from days 31-60 of the year
  bloom$second_30_avgT <- sum(weather$tavg_K[31:60])
  bloom$second_30_maxT <- sum(weather$tmax_K[31:60])
  bloom$second_30_minT <- sum(weather$tmin_K[31:60])
  # bloom$second_30_precip = sum(weather$precip_mm[31:60])
  ...
  # bloom$third_30_precip = sum(weather$precip_mm[61:90])
  
  # Cumulative weather data from days 61-90 of the year
  bloom$fourth_30_avgT <- sum(weather$tavg_K[91:120])
  bloom$fourth_30_maxT <- sum(weather$tmax_K[91:120])
  bloom$fourth_30_minT <- sum(weather$tmin_K[91:120])
  # bloom$fourth_30_precip = sum(weather$precip_mm[91:120])
...
train_data <- dc_bloom[training_samples, 7:22]
test_data <- dc_bloom[-training_samples, 7:22]
...
From the elastic net regression I can see that a ridge regression (alpha = 1) is my best option. However, elastic net chooses a very different lambda value. So I will try ridge regression again using this value to see if I can lower my RMSE and increase my adjusted R^2.
...
```{r}
x_test <- model.matrix(bloom_doy ~ ., ky_bloom[, 7:31])[, -1]
...
I want to see if I can make the prediction better by combining the bloom datasets then running a model over the combined data. 
...
train_data <- blooms[training_samples, c(1,7:31)]
test_data <- blooms[-training_samples, c(1,7:31)]
...
train_data <- blooms[training_samples, c(2,4:5, 7:22)]
test_data <- blooms[-training_samples, c(2,4:5, 7:22)]
...
The model that includes Liestal has a higher root mean squared error than the previous iteration of the model but is still a very accepable value. I would largely attribute that to missing by a lot on some of the outliers in the data set.
...
```{r}
all_loc = rbind(test_data,vc_bloom[,c(2,4:5, 7:31)])
...
dc_weath_comb$alt = 0
dc_weath_comb$rand = rnorm(nrow(dc_weath_comb), sd = .5)
...
ky_weath_comb$rand = rnorm(nrow(ky_weath_comb), sd = .5)
...
li_weath_comb$rand = rnorm(nrow(li_weath_comb), sd = .5)
...
vc_weath_comb$rand = rnorm(nrow(vc_weath_comb), sd = .5)
...
for (i in 1:k) {
  dat <- weath_comb[folds != i, ]
  cv_mod <- glm(tavg_K ~ year + days_into_year + lat + long + alt + rand, data = weath_comb)
  pred <- as.numeric(predict(cv_mod, weath_comb[folds == i, ], id = i, na.rm = T))
  cv.errors[i] <- mean((weath_comb$tavg_K[folds == i] - pred)^2, na.rm = T)
}
# View CV error
...
```{r}
# Create model if CV error is satisfactory
tavg_pred_model <- glm(tavg_K ~ poly(year + days_into_year + lat + long + alt + rand,10), data = weath_comb)
tmax_pred_model <- glm(tmax_K ~ poly(year + days_into_year + lat + long + alt + rand,10), data = weath_comb)
tmin_pred_model <- glm(tmin_K ~ poly(year + days_into_year + lat + long + alt + rand,10), data = weath_comb)
...
dc_weather_pred$rand = rnorm(nrow(dc_weather_pred), sd = .5)
...
```{r}
x_test <- model.matrix(bloom_doy ~ ., dc_bloom_predictors[,c(2,4:5, 7:22)])[, -1]
...
ky_weather_pred$rand = rnorm(nrow(ky_weather_pred), sd = .5)
...
```{r}
x_test <- model.matrix(bloom_doy ~ ., ky_bloom_predictors[,c(2,4:5, 7:22)])[, -1]
...
vc_weather_pred$rand = rnorm(nrow(vc_weather_pred), sd = .5)
...
```{r}
x_test <- model.matrix(bloom_doy ~ ., vc_bloom_predictors[,c(2,4:5, 7:22)])[, -1]
...
li_weather_pred$rand = rnorm(nrow(li_weather_pred), sd = .5)
...
```{r}
x_test <- model.matrix(bloom_doy ~ ., li_bloom_predictors[,c(2,4:5, 7:22)])[, -1]
